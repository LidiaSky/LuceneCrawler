...
import robot.ch2.webcrawler.db.FetchedDocsDB;
import robot.ch2.webcrawler.db.KnownUrlDB;
import robot.ch2.webcrawler.db.ProcessedDocsDB;
import robot.ch2.webcrawler.model.FetchedDocument;
import robot.ch2.webcrawler.model.KnownUrlEntry;
import robot.ch2.webcrawler.model.Outlink;
import robot.ch2.webcrawler.model.ProcessedDocument;
import robot.ch2.webcrawler.parser.common.DocumentParser;
import robot.ch2.webcrawler.parser.common.DocumentParserFactory;
import robot.ch2.webcrawler.transport.common.Transport;
import robot.ch2.webcrawler.transport.file.FileTransport;
import robot.ch2.webcrawler.transport.http.HTTPTransport;
import robot.ch2.webcrawler.utils.DocumentIdUtils;
import robot.ch2.webcrawler.utils.UrlGroup;
import robot.ch2.webcrawler.utils.UrlUtils;
...
public class BasicWebCrawler {
    ...
    private static final int DEFAULT_MAX_BATCH_SIZE = 5;
    
    private long DEFAULT_PAUSE_IN_MILLIS = 50;
    private long pauseBetweenFetchesInMillis = DEFAULT_PAUSE_IN_MILLIS;
    
    /*
     * Число URL- адресов, которые недобходими извлечь
     */
    ...
    public void fetchAndProcess(int maxDepth, int maxDocs) {
        ...
        for(int depth = 0; depth < maxDepth; depth++) {
            ...
            while( maxUrlsReached == false && noMoreUrlsAtThisDepth == false) {
                ...
                /* для каждого пакета адресов создаем отдельную группу документов  */
